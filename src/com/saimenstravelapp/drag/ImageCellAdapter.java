...
public class ImageCellAdapter extends BaseAdapter 
{
	...
	public View getView (int position, View convertView, ViewGroup parent)
	{
		...
		if (convertView == null) {
			...
			GameActivity game = (GameActivity) v.getContext();
		...
		if(gameGUIS.size() > position && gameGUIS.get(position).getResourceId() != -1){
			GameActivity gameActivity = (GameActivity) v.getContext();

			if(gameActivity.getState() == GameActivity.INIT && !gameGUIS.get(position).isDraggable()){
//				Log.d("convertview", "sets not draggable");
				v.draggable = false;
			}

			else if(gameActivity.getState() == GameActivity.YOUR_TURN){
