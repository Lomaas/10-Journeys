package com.saimenstravelapp.drag;
...
implements DragSource, DropTarget
{
	...
	public boolean allowDrag () {
		...
		if(gameActivity.getState() == GameActivity.INIT && this.mGrid != null && this.mEmpty == false && this.draggable == false){		
			return false;
		}

		if(gameActivity.STATE == GameActivity.YOUR_TURN){ 
			if(isYourCardsGrid()){
				Log.d("allowDrag", "test1");
				return false;
			}
		}
		else if(gameActivity.STATE == GameActivity.YOUR_TURN_FROM_DECK){
		...
		else if(gameActivity.STATE == GameActivity.OPPONENTS_TURN){
...
//			Log.d("mCellNumber", Integer.toString(gameActivity.cellTakenFrom));
	...
	public void onDropCompleted (View target, boolean success)
	{
		if(success){
			...
			GameActivity gameActivity = (GameActivity) target.getContext();

			if (mCellNumber >= 0) {
				if(gameActivity.STATE == GameActivity.YOUR_TURN && this.mGrid.getId() == 0){
					gameActivity.setPlayButtonVisible();
				}
				ImageCellAdapter cardAdapter= (ImageCellAdapter) this.mGrid.getAdapter();

				if(gameActivity.STATE == GameActivity.YOUR_TURN_SWAP){
					this.mEmpty = false;
					this.draggable = true;
					gameActivity.setPlayButtonVisible();
				...
				if(gameActivity.cellTakenFrom == this.mCellNumber){
					...
					Drawable d = res.getDrawable (gameActivity.getDrawableIdFromCardId(deletedCardId));	// copy over the drawable
				...
				if(gameActivity.getState() == GameActivity.YOUR_TURN){
					gameActivity.setPlayButtonVisible();
				}
				else if(gameActivity.getState() == GameActivity.INIT)
					gameActivity.setCardButtonEnabled();
	...
	{	
		...
		ImageCellAdapter cardAdapter= (ImageCellAdapter) this.mGrid.getAdapter();
		GameActivity gameActivity = (GameActivity) dragView.getContext();
		
		setBackgroundResource (R.color.cell_filled);
		
		if(gameActivity.getState() == GameActivity.YOUR_TURN_INSERT_TO_OPEN_CARDS){
			gameActivity.setPlayButtonVisible();
			this.draggable = true;
			setBackgroundResource(R.color.cell_empty_hover);
		}
		...
		if(mEmpty == false){			
			if(gameActivity.getState() == GameActivity.INIT){
				...
				gameActivity.setState(GameActivity.YOUR_TURN_SWAP);
	...
	public boolean acceptDrop(DragSource source, int x, int y, int xOffset, int yOffset,
		...
		if(this.mGrid != null){		
			...
			if(gameActivity.STATE == GameActivity.INIT){
			...
			else if(gameActivity.STATE == GameActivity.YOUR_TURN_INSERT_TO_OPEN_CARDS){
				if(isYourCardsGrid())
					return false;
				if(this == cell)
					return false;
			...
			else if(gameActivity.STATE == GameActivity.YOUR_TURN_FROM_DECK || gameActivity.STATE == GameActivity.YOUR_TURN){
				...
				gameActivity.setAllButtonUnvisible();
			}

			// From gridsforcardsOnTable set celltakenFrom
			if(gameActivity.STATE == GameActivity.YOUR_TURN && cell.getFromGridId() == R.id.gridForCardsOnTable){
				gameActivity.cellTakenFrom = cell.mCellNumber;
			}
		}
